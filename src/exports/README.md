...
3. Get access to the API with the following code:
    ...
    ```ts
    const clineExtension = vscode.extensions.getExtension<ClineAPI>("iabacusai.aaca-cline-dev")
    ...
    **Note:** To ensure that the `iabacusai.aaca-cline-dev` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

    ```json
    "extensionDependencies": [
        "iabacusai.aaca-cline-dev"
    ]
    ```
